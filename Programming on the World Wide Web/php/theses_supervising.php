...
if ($user_id > 0) {
    $query = "SELECT th.*, CONCAT(stud.name, ' ', stud.surname) AS student_name, CONCAT(sup.name, ' ', sup.surname) AS supervisor_name, CONCAT(co_sup1.name, ' ', co_sup1.surname) AS supervisor2_name, CONCAT(co_sup2.name, ' ', co_sup2.surname) AS supervisor3_name, venue, evaluation_report, presentation_date, protocol_number
              FROM theses th LEFT JOIN students stud ON th.student_id = stud.AM  LEFT JOIN users sup ON th.supervisor_id = sup.user_id LEFT JOIN users co_sup1 ON th.supervisor2_id = co_sup1.user_id LEFT JOIN users co_sup2 ON th.supervisor3_id = co_sup2.user_id WHERE 1=1";

    // supervisor1 or co
    $query .= " AND (supervisor_id = $user_id OR supervisor2_id = $user_id OR supervisor3_id = $user_id)";
    ...
    if ($role === 'supervisor1') {
        ...
        $query .= " AND (supervisor2_id = $user_id OR supervisor3_id = $user_id)";
    }
    ...
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            $row['role'] = ($row['supervisor_id'] == $user_id) ? 'supervisor1' : 'Committee Member';
